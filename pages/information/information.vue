<template>
	<view class="container">
		...
		<uni-popup ref="emailPopup" type="center" animation="ture">
			<view class="last-box">
				<view class="phone-box">
					<input type="text" pla ceholder="请输入邮箱 " class="phoneNumber" v-model="emailInput" />
				</view>
		...
		methods: {
			...
			getUserData() {
				...
				request.baseRequest(opts, param).then(res => {
					...
					this.$store.state.phone = this.phoneNumber //放到vueX,提现时使用
				}, error => {
					console.log(error);
				})
			},
			...
			qrCode() {
				...
				let opts = {
					url: 'qrcode/getwxacodeunlimit', //接口地址
					method: 'POST' //请求类型
				};
				let param = {
					openid: id
